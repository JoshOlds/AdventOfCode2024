// High level concept for the day is to create a Dictionary (Map) for every number (key) that has a rule. The values in the dictionary
// will be two lists (HashSet) - one list will be the numbers that must come before the key number, and the
// other list is numbers that must come after the key number. ('before numbers' and 'after number)'
// To evaluate if a list of pages is valid, iterate through the list. For each number, look up the entry for that number
// in the rules dictionary (by key), and get the list of 'before numbers' and 'after numbers' for that key.
// Check that these numbers do indeed only come before or after the current key number in the list of pages.

// For Part 2, my approach was to iterate through the list of pages and evaluate the 'before numbers' and 
// 'after numbers' (as done in part 1). The difference, however, is when we find a number that breaks the rule, that 
// number is immediately moved in the list to no longer break the rule (delete and insert it either before or after the 
// current number, depending on the rule). 
// The danger here is that we are manipulating a list while iterating through it. To protect ourselves, I set this function
// up to recurse after making a single edit. This ensures that we iterate through the list fresh after every edit, rather
// than continuing to iterate after making a modification. We recurse until no more rules are broken and the list should 
// now be in a fixed state.

using Godot;
using System;
using System.Collections.Generic;
using System.Linq;

/// A set of rules that dictate numbers that must come before and after the number that this rule belongs to.
struct NumberRule
{
	public NumberRule()
	{
		numbersThatComeBefore = new HashSet<int>();
		numbersThatComeAfter = new HashSet<int>();
	}
	/// A HashSet (unique list) of all numbers that must come before the number that this rule belongs to.
	public HashSet<int> numbersThatComeBefore;
	/// A HashSet (unique list)  of all numbers that must come after the number that this rule belongs to.
	public HashSet<int> numbersThatComeAfter;
}

public partial class day5 : Node
{
	/// Dictionary that associates a NumberRule with a single numeric key. 
	private Dictionary<int, NumberRule> ruleDict;
	private List<List<int>> invalidPages;

	// Called when the node enters the scene tree for the first time.
	public override void _Ready()
	{
		// Initialize lists and dicts
		ruleDict = new Dictionary<int, NumberRule>();
		invalidPages = new List<List<int>>();
		
		GD.Print("Day 5 Part 1: ");
		ExecutePart1();
		GD.Print("\nDay 5 Part 2: ");
		ExecutePart2();
	}

	public void ExecutePart1()
	{
		PopulateNumberRuleDictionary();
		List<List<int>> pagesLists = GetPagesLists();
		int validSum = 0;
		foreach (List<int> pages in pagesLists)
		{
			int result = evaluatePageRules(pages);
			if (result > -1) validSum += result;
		}
		GD.Print(validSum);
	}

	public void ExecutePart2()
	{
		foreach (List<int> pages in invalidPages)
		{
			FixInvalidPages(pages);
		}

		int sum = 0;
		foreach (List<int> pages in invalidPages)
		{
			sum += pages[pages.Count / 2];
		}
		GD.Print(sum);
	}

	/// Iterate through the rules in the puzzle input and create entries into the ruleDict.
	private void PopulateNumberRuleDictionary()
	{
		string rulesString = GetPuzzleRules();
		List<string> rulesLines = rulesString.Split("\n").ToList();
		foreach (string line in rulesLines)
		{
			int beforeInt = line.Split("|")[0].ToInt();
			int afterInt = line.Split("|")[1].ToInt();
			AddToRuleDict(beforeInt, afterInt);
		}
	}

	/// For every given pair of numbers, this function will make two entries into the rulesDict. One for each number as the key.
	private void AddToRuleDict(int beforeNumber, int afterNumber)
	{
		// Create the Dictionary entry if a rule does not already exist for this number
		if (ruleDict.ContainsKey(beforeNumber) == false)
		{
			NumberRule newRule = new NumberRule();
			ruleDict.Add(beforeNumber, newRule);
		}
		// Add the new 'after number' to the rule HashSet
		NumberRule beforeRule = ruleDict[beforeNumber];
		beforeRule.numbersThatComeAfter.Add(afterNumber);
		
		// Do the same thing for the After number (add to the NumbersThatComeBefore)
		if (ruleDict.ContainsKey(afterNumber) == false)
		{
			NumberRule newRule = new NumberRule();
			ruleDict.Add(afterNumber, newRule);
		}
		// Add the new before number to the rule HashSet
		NumberRule afterRule = ruleDict[afterNumber];
		afterRule.numbersThatComeBefore.Add(beforeNumber);
	}

	/// Evaluates if a list of pages is valid based on the puzzle input rules. If so, returns the middle value of the list.
	private int evaluatePageRules(List<int> pages)
	{
		bool validPageOrder = true;
		
		// Iterate through each page, and check if any before or after rules are broken
		for (int i = 0; i < pages.Count; i++)
		{
			int pageNumber = pages[i];
			if (ruleDict.ContainsKey(pageNumber) == false) continue;
			
			List<int> previousPages = GetPreviousPages(pages, i);
			List<int> followingPages = GetFollowingPages(pages, i);
			NumberRule rule = ruleDict[pageNumber];
			
			// Check if any pages that must come before are in the following list (invalid)
			foreach (int beforeNumber in rule.numbersThatComeBefore)
			{
				if (followingPages.Contains(beforeNumber)) validPageOrder = false;
			}
			// Check if any pages that must come after are in the previous list (invalid)
			foreach (int afterNumber in rule.numbersThatComeAfter)
			{
				if (previousPages.Contains(afterNumber)) validPageOrder = false;
			}
		}

		if (validPageOrder == false)
		{
			invalidPages.Add(pages); // Store invalid pages for use in part 2 of the puzzle
			return -1;
		}
		return pages[pages.Count / 2]; // Middle page number
	}

	/// Recursive function that moves list items in-place to fix rule violations
	private void FixInvalidPages(List<int> invalidPages)
	{
		for (int i = 0; i < invalidPages.Count; i++)
		{
			int pageNumber = invalidPages[i];
			if (ruleDict.ContainsKey(pageNumber) == false) continue; // Move on if no rules exist for this number
			
			List<int> previousPages = GetPreviousPages(invalidPages, i);
			List<int> followingPages = GetFollowingPages(invalidPages, i);
			NumberRule rule = ruleDict[pageNumber];
			
			// Find and move any page that should come before, but is found in the followingPages list
			foreach (int beforeNumber in rule.numbersThatComeBefore)
			{
				if (followingPages.Contains(beforeNumber))
				{
					invalidPages.Remove(beforeNumber);
					invalidPages.Insert(i, beforeNumber); // Move the number before our current index
					FixInvalidPages(invalidPages); // !! Recurse !!
					return;
				}
			}
			// Find and move any page that should come after, but is found in the previousPages list
			foreach (int afterNumber in rule.numbersThatComeAfter)
			{
				if (previousPages.Contains(afterNumber))
				{
					invalidPages.Remove(afterNumber);
					invalidPages.Insert(i + 1, afterNumber); // Move the number after our current index
					FixInvalidPages(invalidPages); // !! Recurse !!
					return;
				}
			}
			
		}
	}

	// Accepts a pageList (list of ints) and returns list of pages that come before a given index
	private List<int> GetPreviousPages(List<int> pagesList, int currentPageIndex)
	{
		List<int> previousPages = new List<int>();
		for (int i = currentPageIndex - 1; i >= 0; i--)
		{
			previousPages.Add(pagesList[i]);
		}

		return previousPages;
	}
	
	// Accepts a pageList (list of ints) and returns list of pages that come after a given index
	private List<int> GetFollowingPages(List<int> pagesList, int currentPageIndex)
	{
		List<int> followingPages = new List<int>();
		for (int i = currentPageIndex + 1; i < pagesList.Count; i++)
		{
			followingPages.Add(pagesList[i]);
		}

		return followingPages;
	}

	private List<List<int>> GetPagesLists()
	{
		string pagesString = GetPuzzlePages();
		List<string> lines = pagesString.Split("\n").ToList();
		List<List<int>> pagesLists = new List<List<int>>();

		foreach (string line in lines)
		{
			List<int> pages = new List<int>();
			List<string> nums = line.Split(",").ToList();
			foreach (string num in nums)
			{
				pages.Add(num.ToInt());
			}
			pagesLists.Add(pages);
		}

		return pagesLists;
	}

	private static string GetPuzzleRules()
	{
		return @"56|54
94|55
94|11
42|41
42|21
42|32
54|13
54|42
54|61
54|98
26|95
26|79
26|11
26|14
26|24
38|41
38|74
38|54
38|32
38|39
38|94
16|37
16|38
16|55
16|14
16|29
16|44
16|24
95|88
95|62
95|32
95|34
95|11
95|91
95|57
95|65
57|42
57|39
57|14
57|65
57|17
57|61
57|19
57|21
57|64
76|97
76|57
76|95
76|15
76|44
76|54
76|11
76|61
76|64
76|91
35|58
35|94
35|32
35|79
35|91
35|16
35|39
35|57
35|62
35|21
35|17
98|38
98|95
98|35
98|76
98|56
98|23
98|62
98|97
98|55
98|67
98|88
98|33
62|17
62|15
62|16
62|32
62|58
62|65
62|21
62|61
62|54
62|26
62|73
62|79
62|74
84|73
84|75
84|74
84|44
84|27
84|15
84|76
84|34
84|37
84|88
84|35
84|62
84|91
84|33
65|41
65|24
65|58
65|79
65|26
65|13
65|23
65|87
65|19
65|67
65|14
65|29
65|98
65|39
65|21
88|57
88|54
88|62
88|38
88|21
88|17
88|37
88|46
88|39
88|73
88|42
88|25
88|19
88|74
88|61
88|65
29|97
29|11
29|91
29|35
29|84
29|57
29|75
29|76
29|38
29|34
29|15
29|37
29|33
29|44
29|56
29|27
29|88
15|24
15|16
15|26
15|42
15|19
15|64
15|14
15|28
15|58
15|39
15|17
15|67
15|61
15|65
15|23
15|13
15|32
15|41
19|87
19|21
19|39
19|23
19|76
19|13
19|34
19|24
19|79
19|58
19|14
19|75
19|95
19|29
19|41
19|44
19|98
19|94
19|16
58|27
58|87
58|84
58|11
58|55
58|97
58|23
58|67
58|98
58|79
58|34
58|95
58|76
58|56
58|13
58|28
58|94
58|75
58|24
58|14
17|84
17|76
17|19
17|23
17|75
17|28
17|29
17|58
17|94
17|13
17|41
17|16
17|87
17|24
17|67
17|14
17|21
17|55
17|79
17|39
17|26
33|39
33|19
33|54
33|62
33|94
33|58
33|13
33|35
33|61
33|21
33|26
33|41
33|57
33|65
33|17
33|64
33|42
33|25
33|79
33|74
33|73
33|91
11|32
11|62
11|57
11|46
11|35
11|74
11|38
11|39
11|61
11|33
11|42
11|17
11|15
11|19
11|41
11|64
11|21
11|54
11|91
11|73
11|25
11|65
11|88
44|56
44|32
44|88
44|38
44|11
44|91
44|35
44|74
44|54
44|95
44|46
44|57
44|97
44|65
44|33
44|42
44|27
44|61
44|73
44|15
44|37
44|62
44|64
44|34
39|67
39|94
39|34
39|26
39|97
39|76
39|44
39|58
39|79
39|24
39|87
39|95
39|27
39|55
39|29
39|16
39|84
39|28
39|56
39|23
39|75
39|98
39|13
39|14
91|28
91|73
91|16
91|58
91|57
91|54
91|74
91|21
91|41
91|62
91|15
91|26
91|94
91|61
91|19
91|25
91|32
91|79
91|42
91|65
91|17
91|13
91|39
91|64
74|54
74|16
74|64
74|57
74|13
74|26
74|15
74|42
74|94
74|61
74|39
74|17
74|32
74|19
74|41
74|79
74|21
74|58
74|73
74|28
74|98
74|25
74|65
74|14
28|11
28|84
28|98
28|37
28|46
28|76
28|38
28|44
28|88
28|24
28|29
28|33
28|67
28|95
28|56
28|55
28|35
28|27
28|97
28|23
28|14
28|34
28|75
28|87
13|24
13|38
13|46
13|23
13|44
13|98
13|87
13|75
13|27
13|84
13|88
13|67
13|95
13|97
13|11
13|28
13|76
13|56
13|29
13|34
13|37
13|14
13|55
13|16
37|39
37|74
37|15
37|41
37|21
37|58
37|33
37|73
37|26
37|64
37|54
37|25
37|65
37|19
37|57
37|61
37|42
37|17
37|32
37|38
37|91
37|46
37|35
37|62
24|46
24|33
24|57
24|62
24|56
24|54
24|84
24|29
24|44
24|97
24|76
24|88
24|87
24|11
24|38
24|95
24|27
24|37
24|34
24|55
24|74
24|75
24|35
24|91
97|15
97|35
97|54
97|88
97|57
97|64
97|19
97|37
97|41
97|21
97|25
97|62
97|91
97|38
97|73
97|32
97|74
97|46
97|42
97|33
97|65
97|61
97|11
97|17
25|13
25|67
25|58
25|19
25|28
25|24
25|75
25|95
25|39
25|16
25|44
25|26
25|41
25|84
25|55
25|79
25|94
25|87
25|98
25|76
25|29
25|23
25|21
25|14
79|75
79|56
79|94
79|23
79|13
79|16
79|76
79|88
79|98
79|27
79|87
79|14
79|24
79|84
79|67
79|28
79|97
79|29
79|37
79|44
79|34
79|55
79|11
79|95
27|15
27|41
27|17
27|11
27|19
27|88
27|64
27|32
27|62
27|65
27|46
27|91
27|38
27|42
27|25
27|35
27|97
27|54
27|73
27|57
27|37
27|74
27|33
27|61
61|16
61|28
61|65
61|24
61|23
61|25
61|19
61|79
61|42
61|39
61|13
61|67
61|32
61|84
61|21
61|41
61|29
61|17
61|94
61|14
61|98
61|55
61|58
61|26
87|56
87|38
87|61
87|73
87|33
87|44
87|54
87|35
87|97
87|88
87|62
87|57
87|95
87|74
87|11
87|75
87|34
87|37
87|76
87|15
87|91
87|64
87|27
87|46
34|27
34|11
34|97
34|91
34|54
34|74
34|42
34|33
34|88
34|56
34|57
34|64
34|17
34|35
34|32
34|61
34|46
34|25
34|38
34|65
34|73
34|37
34|15
34|62
14|35
14|24
14|27
14|56
14|75
14|29
14|87
14|55
14|95
14|76
14|84
14|98
14|97
14|33
14|88
14|91
14|38
14|44
14|46
14|23
14|37
14|11
14|34
14|67
21|44
21|16
21|14
21|67
21|98
21|84
21|27
21|94
21|26
21|76
21|79
21|55
21|75
21|13
21|56
21|28
21|24
21|58
21|39
21|95
21|34
21|29
21|87
21|23
75|88
75|95
75|64
75|35
75|54
75|56
75|27
75|57
75|15
75|46
75|61
75|74
75|33
75|76
75|11
75|73
75|44
75|97
75|42
75|37
75|91
75|38
75|62
75|34
23|55
23|76
23|37
23|44
23|11
23|74
23|84
23|29
23|75
23|33
23|97
23|57
23|95
23|88
23|38
23|27
23|91
23|56
23|46
23|34
23|24
23|62
23|87
23|35
64|32
64|98
64|41
64|25
64|28
64|21
64|61
64|17
64|39
64|24
64|29
64|65
64|19
64|42
64|16
64|13
64|23
64|79
64|58
64|26
64|84
64|94
64|67
64|14
41|16
41|98
41|13
41|67
41|58
41|29
41|87
41|23
41|94
41|28
41|26
41|75
41|84
41|21
41|95
41|44
41|79
41|14
41|39
41|24
41|76
41|34
41|55
41|56
32|41
32|75
32|87
32|29
32|13
32|28
32|24
32|26
32|84
32|17
32|94
32|67
32|21
32|14
32|19
32|98
32|58
32|55
32|79
32|16
32|25
32|39
32|23
32|76
55|56
55|97
55|46
55|27
55|37
55|64
55|11
55|44
55|54
55|57
55|15
55|87
55|74
55|73
55|91
55|75
55|62
55|38
55|33
55|35
55|76
55|34
55|95
55|88
46|79
46|38
46|62
46|35
46|65
46|19
46|91
46|74
46|32
46|15
46|54
46|42
46|73
46|57
46|17
46|25
46|41
46|64
46|58
46|61
46|26
46|39
46|21
46|33
73|98
73|19
73|61
73|79
73|41
73|39
73|64
73|24
73|14
73|16
73|65
73|32
73|42
73|17
73|94
73|25
73|28
73|67
73|13
73|58
73|26
73|29
73|23
73|21
67|37
67|84
67|11
67|97
67|24
67|74
67|55
67|34
67|46
67|91
67|44
67|27
67|29
67|88
67|75
67|35
67|62
67|76
67|33
67|38
67|23
67|95
67|56
67|87
56|33
56|65
56|37
56|35
56|62
56|73
56|91
56|19
56|11
56|25
56|15
56|32
56|38
56|74
56|46
56|27
56|17
56|88
56|64
56|61
56|97
56|42
56|57
94|14
94|87
94|88
94|95
94|23
94|97
94|44
94|28
94|37
94|56
94|84
94|46
94|27
94|13
94|76
94|34
94|98
94|16
94|67
94|24
94|29
94|75
42|29
42|13
42|58
42|65
42|19
42|23
42|98
42|26
42|39
42|87
42|84
42|24
42|25
42|79
42|14
42|16
42|28
42|17
42|55
42|67
42|94
54|23
54|21
54|39
54|26
54|14
54|64
54|28
54|32
54|17
54|25
54|41
54|16
54|15
54|19
54|65
54|67
54|94
54|79
54|58
54|73
26|98
26|27
26|75
26|67
26|94
26|84
26|13
26|87
26|28
26|23
26|16
26|76
26|34
26|56
26|29
26|55
26|88
26|97
26|44
38|15
38|57
38|33
38|25
38|26
38|58
38|64
38|79
38|17
38|35
38|19
38|73
38|42
38|61
38|21
38|62
38|91
38|65
16|95
16|34
16|46
16|27
16|67
16|88
16|87
16|56
16|23
16|11
16|98
16|84
16|76
16|33
16|97
16|28
16|75
95|27
95|15
95|97
95|17
95|73
95|56
95|46
95|42
95|74
95|35
95|38
95|33
95|54
95|61
95|64
95|37
57|26
57|73
57|28
57|67
57|41
57|54
57|58
57|32
57|98
57|79
57|94
57|25
57|13
57|15
57|16
76|37
76|34
76|56
76|74
76|38
76|35
76|65
76|33
76|27
76|46
76|62
76|73
76|88
76|42
35|54
35|61
35|74
35|64
35|73
35|42
35|19
35|15
35|13
35|26
35|65
35|41
35|25
98|24
98|37
98|87
98|27
98|84
98|46
98|34
98|29
98|44
98|91
98|75
98|11
62|39
62|14
62|42
62|57
62|41
62|28
62|94
62|19
62|25
62|64
62|13
84|38
84|97
84|55
84|87
84|56
84|46
84|54
84|57
84|95
84|11
65|17
65|75
65|32
65|84
65|28
65|94
65|55
65|16
65|25
88|33
88|41
88|91
88|32
88|58
88|64
88|35
88|15
29|95
29|55
29|54
29|87
29|74
29|62
29|46
15|94
15|73
15|21
15|79
15|25
15|98
19|84
19|67
19|28
19|55
19|26
58|26
58|44
58|16
58|29
17|44
17|25
17|98
33|32
33|15
11|37";
	}

	private static string GetPuzzlePages()
	{
		return @"38,33,35,91,62,74,57,54,15,73,64,61,65,32,25,19,21,39,58,26,79
46,62,56,61,35,88,74
24,17,55,67,42,16,58
16,39,24,25,28,17,98,41,64,21,29,42,13,79,61,19,14,58,67,94,32
34,56,97,37,38,33,35,91,62,54,15,73,64,42,65,32,17
37,46,38,33,91,62,74,57,54,15,73,64,61,42,65,32,17,25,19,41,21,39,58
65,32,17,25,41,21,39,58,26,79,94,13,16,28,14,98,23,29,84,55,87
17,25,41,21,58,79,94,13,16,14,98,67,29,55,76
29,21,25,19,98,94,42,28,26,17,24,55,67,79,39,14,32,23,84,58,13,41,65
32,42,21,39,79,13,41,94,28
98,67,23,24,29,84,55,87,75,44,95,34,97,11,37,38,91
33,94,54,32,42,62,19
13,95,28,44,24,76,79,39,84,29,98,87,19
41,21,39,58,26,79,94,13,16,28,14,98,67,23,24,29,84,55,75,76,44,95,34
58,61,21,13,84,42,79,26,65,14,94,41,23,29,39
57,15,32,25,21,65,39,79,58,19,73,91,54,26,64,17,61,42,35,33,74
91,29,75,46,55,11,76,35,84,97,24,95,74,57,33,38,62
73,55,91,11,33,87,54
55,24,14,16,23,94,44,98,34,29,76,87,28,84,79,97,95,11,27,26,56
67,24,55,95,56,27,97,11,88,37,46,38,35,91,62
16,28,75,21,24,95,84,29,19,44,67,76,87,98,14
17,25,19,41,21,39,58,79,94,13,16,14,98,67,23,24,84,55,87,75,76
21,39,26,79,94,13,16,14,98,67,23,24,84,75,76,44,95,34,56
97,11,88,46,38,33,35,91,62,74,57,54,15,73,64,61,42,65,32,17,25,19,41
75,95,27,11,88,37,46,35,91,62,74,57,54,15,73,64,61
32,17,25,19,41,21,39,58,26,79,94,13,16,14,98,67,23,24,29,84,55,87,75
91,57,15,61,42,17,25,41,21
95,56,97,11,37,46,38,33,35,91,62,74,57,54,15,73,42,65,32
38,35,54,88,15,19,39,46,64,65,21,57,73,17,91
29,84,76,97,37,91,54
98,67,29,84,55,87,75,76,44,95,34,56,27,97,88,37,46,38,33,35,91
54,15,73,64,65,32,17,25,19,41,21,39,58,26,16,98,67
94,13,16,28,98,67,23,29,84,55,87,75,76,44,95,34,56,27,37
13,16,28,14,98,67,23,24,84,55,87,76,44,56,27,97,11,37,46
32,21,94,13,28,24,87
16,14,76,44,97,46,38
88,25,62,19,65,15,32,57,46,54,39,38,91,42,37,41,35
95,56,11,88,46,33,91,62,74,57,54,15,73,64,61,42,32
75,95,56,27,97,37,46,33,35,91,62,57,54,73,61
29,84,55,87,44,95,34,27,11,88,37,46,38,62,54
27,11,91,61,65,32,17
76,44,95,34,56,27,97,88,37,46,38,33,91,62,74,57,54,15,73,61,42
54,73,64,61,65,32,25,19,41,21,39,58,94,13,16,98,67
28,23,84,76,44,88,37,46,33
14,84,55,87,44,88,35
57,11,56,84,75,35,33,54,15,27,95,34,55
61,46,38,91,73,11,17,64,65,74,35,57,37,34,32
14,23,24,29,84,55,75,76,95,34,56,27,97,88,37,46,38,33,35
91,33,15,62,64,41,17,32,54,42,25,65,35,61,88,38,74,37,57,97,73
27,97,33,62,74,54,64,65,19
44,34,88,37,54,64,65
16,56,44,26,14,76,34,94,28,75,27,95,67,98,23,79,87,97,58
42,21,32,94,41,79,26,25,57,65,33,62,54,15,91,74,35
62,54,19,39,16
97,61,44,54,62,95,56,46,27,37,34,57,88,38,91
61,65,17,25,19,94,13,28,84
35,64,32,42,37,65,25,57,33,73,17,11,46,19,54,62,88,41,38
73,25,61,42,17,41,62,35,88,65,54
87,76,44,95,56,11,88,37,46,33,91,62,15,73,64
54,15,39,62,21,32,61,46,41,74,33
76,95,34,88,37,46,33,35,62,57,54,15,64,61,42
25,57,65,37,17,56,15
21,58,26,16,14,67,29,84,76,44,56
74,57,54,15,73,64,61,42,65,32,17,25,19,41,39,58,26,79,94,13,16,28,14
23,24,84,55,76,95,27,97,11,88,37,46,38,35,91,62,74
26,73,21,25,35,54,74,17,64,94,62
16,61,19,23,65,32,25,39,15
32,25,19,41,21,58,26,94,13,16,14,98,67,23,24,29,84,55,75
34,55,95,75,57,56,97,38,11,84,29,76,91,37,74,87,62,24,27,33,35
21,55,25,67,17,16,29,19,98,58,24,32,26,87,84,75,23,41,14
61,28,94,14,25,79,65,98,41,67,26,21,13,24,19,32,58,29,39,84,42
42,32,17,25,19,41,21,58,94,13,28,14,98,67,23,24,29
32,26,87,21,39,41,79,55,98,67,24,17,16,75,28
84,21,28,67,87,29,76,79,95,41,14,55,39,23,26,44,13,16,94,98,24
39,79,94,13,28,98,67,23,24,29,84,55,87,75,76,95,34,56,27
34,15,57,76,91,75,37,38,54,55,27,44,33,46,88,87,62,73,11
55,87,75,76,95,34,27,97,11,88,38,35,91,74,57,54,73
97,37,38,62,74,57,15,73,64,42,17,25,41
95,34,56,97,11,88,37,46,33,35,91,74,57,15,73,64,61,65,32
21,58,26,13,16,28,14,98,67,23,24,29,84,87,76,44,95,34,56
97,98,11,38,87,23,16
26,19,84,76,58,79,94,25,23,28,98,87,41,39,75,13,44
95,34,56,27,97,11,88,37,46,38,91,62,74,57,15,64,61,42,65
75,44,95,34,56,11,88,37,35,73,64
15,73,64,61,42,32,17,25,19,21,39,58,26,79,94,13,16,28,14,98,23
79,13,16,14,98,23,24,29,84,55,87,75,76,95,34,56,11
94,13,16,28,98,24,29,84,55,87,75,76,95,34,27,97,11
94,98,67,76,56,97,37
19,21,39,13,16,23,24,29,84,55,75,76,95
54,65,32,17,25,26,79,13,16,14,67
27,91,74,57,65,17,19
38,33,35,91,62,54,15,64,61,42,65,32,17,25,19,41,21,39,79
16,28,23,24,55,87,44,34,56,27,97,11,37,46,38
54,15,44,74,65,38,95,62,73,64,42,33,34
16,14,98,67,24,29,55,87,75,76,44,97,11,88,37,46,38
73,57,42,54,27,33,11,17,37,64,65,62,97
91,57,54,15,73,64,42,65,17,25,19,21,39,26,79,94,16
41,39,58,26,79,94,13,16,28,14,98,67,23,24,29,84,55,87,75,76,44,95,34
28,14,98,67,23,24,29,84,55,87,75,76,44,95,34,56,27,11,88,37,46,38,33
46,33,74,88,64,62,27,37,65,38,44
61,42,65,32,17,25,19,41,39,58,26,79,94,13,16,28,98,67,24,29,84
64,14,39,32,61,28,57,13,25,16,17,65,94,58,21,41,19,79,98,15,73,26,54
91,46,65,26,19,74,41
23,24,29,84,55,87,75,76,44,95,34,56,27,11,37,46,38,33,35,62,74
64,61,42,65,32,17,25,19,41,21,39,58,26,79,94,13,16,28,14,98,67,24,29
19,21,13,16,23,75,95
37,46,15,73,64,61,32
87,97,46,38,57
38,57,46,75,29,91,56,97,11,55,84,27,76,37,54,87,44,74,95
73,61,42,65,32,39,24
24,29,84,55,75,95,34,56,27,97,11,88,37,46,33,91,62,74,57
79,94,28,98,67,23,24,84,55,87,76,95,34,56,88
33,27,62,64,61,88,95,97,54,74,56
98,46,97,16,88,95,56,14,13
15,17,61,73,88,41,19,25,37,62,64,35,54,65,33,11,46,57,97
13,14,23,87,95,27,11
55,13,25,79,29,14,42,94,84,39,24,67,19
88,29,34,67,28,23,79,87,24
41,21,39,58,26,79,94,13,16,14,98,67,23,24,29,84,55,87,75,76,44,95,34
16,65,17,19,25
64,97,25,57,35,54,74,19,46,65,17,32,11,91,61,15,62,73,41,42,33,88,38
65,32,17,25,39,58,26,14,98,23,29,84,87
57,55,95,56,76,62,75,15,74,27,88,73,54,87,37,46,38
17,11,46,25,21,91,73
67,87,75,76,34,46,35,91,62
57,37,33,65,42,61,58,39,17,41,64,21,15,32,46,25,19,35,73,91,62
17,28,39,58,19,32,24,41,13,14,23,26,29,98,16,84,42
21,67,23,41,64,98,13,17,16,24,32,73,14,26,19
55,16,95,21,56,26,94,79,58
76,44,33,91,74
55,87,75,44,34,27,88,37,33,91,62,74,54,15,73
39,41,54,88,19,62,61
42,64,21,79,17,25,98,57,39,41,28,54,61,94,15,32,19,58,14,13,73
65,95,61,42,73,34,38,56,64,54,44
25,64,28,39,16,62,32
21,39,58,79,13,16,24,84,95,34,56
34,97,88,91,62,74,54,15,73,61,17
73,46,54,88,74,61,15,33,11,62,65,37,41,42,17,38,21,19,57,32,25,35,64
91,54,15,73,64,61,42,65,17,19,21,39,79
75,76,56,27,97,11,37,38,35,91,57,15,61
41,21,39,26,94,13,16,28,14,98,67,23,29,84,55,87,75,76,44,95,34
28,14,67,23,29,84,55,87,75,76,95,34,27,97,11,88,46,38,33
95,97,46,91,62,74,57,73,61,65,32
94,13,16,28,14,98,67,23,24,29,55,87,75,76,44,95,34,56,27,97,11,88,37
67,24,29,84,95,56,11,88,37,46,33,35,91
37,46,38,33,62,74,57,15,73,61,42,19,58
19,21,39,58,79,13,98,76,95
27,79,94,24,87,75,88,55,14
91,88,29,34,98,75,56,23,97,33,95,84,27,46,67,44,87,38,76,37,35
79,16,28,98,23,84,75,97,88
23,13,39,28,76,16,56,26,98,44,34,87,84,29,58,79,27
37,46,38,33,35,91,74,57,54,15,64,42,65,32,17,25,19,41,21,39,58
33,97,76,14,29,98,28,34,56,27,87,38,88,24,23,11,46
28,23,55,87,76,34,56,11,88,37,46,38,33
62,74,73,64,61,42,65,41,21,39,79,94,28
62,74,57,54,15,64,61,42,65,32,17,25,21,39,58,26,79,94,13,16,28
26,79,16,28,14,24,84,87,76,95,11
67,23,24,84,55,87,75,44,34,46,33,91,62
41,21,39,58,26,79,94,13,28,14,98,67,23,24,29,84,87,75,76,44,34
16,67,24,29,55,34,11
27,98,37,91,55,44,95
91,62,74,57,15,73,64,61,42,17,19,41,39,26,94,13,16
26,98,29,84,55,95,27
55,87,76,44,95,34,56,27,97,88,38,33,35,91,62
54,91,42,79,57,62,35
26,79,13,14,98,67,24,84,55,75,44,95,34,97,11
28,98,19,26,39,75,84,32,67,29,55,24,17,41,23
33,35,91,62,57,54,15,73,64,61,42,65,32,17,25,19,41,21,39,58,26,79,94
15,98,42,65,64,17,94,28,58,39,57,32,25,41,73,19,16
16,98,67,23,29,84,55,87,75,44,95,34,56,27,11,88,37,46,38
17,73,56,42,11,34,37,46,61,97,54
29,84,75,56,37,62,54
37,46,38,33,35,62,74,57,54,15,73,64,61,42,65,32,17,25,19,41,21,39,58
91,21,13,41,64,57,15,42,26,79,19,65,32,54,35,25,94,39,62,73,58,17,74
62,74,54,15,42,65,17,25,19,41,39,16,28
91,62,74,57,64,42,65,32,19,21,39,26,79,94,16
75,88,46,76,11,38,67,95,33,91,97,23,56,44,34,62,87,84,29
73,16,79,67,26,65,21,94,64,28,61,39,58,19,15
33,35,91,62,74,57,73,65,25,19,41,21,94
75,76,44,95,34,56,27,97,11,88,46,38,35,91,62,54,73,64,61
41,21,94,13,14,67,23,84,55,87,75,44,34
65,25,28,98,87
16,28,14,98,29,55,87,75,97,37,38
55,76,95,97,88,37,38,35,62,57,15
17,25,19,41,21,39,58,26,79,94,13,16,28,14,98,23,24,29,84,55,87,75,76
65,25,41,21,39,79,94,14,67,24,84,55,87
61,19,13,14,29
24,29,84,87,75,95,34,11,37,46,35,91,62,74,57
34,56,27,97,88,37,46,33,91,62,74,57,54,15,73,64,61,42,65,32,17
15,64,42,32,17,25,41,21,39,58,26,79,94,13,16,28,67
21,58,16,14,84,87,75
46,33,35,62,54,73,65,17,25,39,26
56,97,37,46,38,35,91,62,57,54,15,61,65,32,17
19,15,79,98,64,21,16,67,42,73,17,13,26,28,41";
	}

}
